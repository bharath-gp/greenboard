...
module.exports = function () {
    ...
    function _db(bucket) {
        ...
        //}
        var db = cluster.bucket(bucket);
        const collection = db.defaultCollection();
        //let db = cluster.openBucket(bucket)
    ...
    function _query(query_string) {
        ...
        var promise = new Promise(function (resolve, reject) {
            const _cluster = new couchbase.Cluster(config.Cluster,
                {
                    username: config.RBACUser,
                    password: config.RBACKPassword,
                    kvTimeout: 30*1000,
            ...
            const b = _cluster.bucket('greenboard').defaultCollection()
            _cluster.query(query_string,function (err, components) {
                if (!err) {
                    resolve(components.rows)
                } else {
                    reject(err)
                }
            })
    ...
    function _getmulti(bucket, docIds) {
        try {
            ...
            for(const docId of docIds) {
                ...
                funs.push(db.get(docId, options));
            }
    ...
    function _get(bucket, documentId) {
        ...
        db.get(documentId, (error, result) =>
        {
            if(error){
    ...
    var API = {
        ...
        jobsForBuild: function (bucket, build) {
            function getJobs() {
                ...
                let result = {};
                return _getmulti('greenboard', [doc_id, existing_builds_id])
                    .then((docs) => {
                        console.log(docs);
                        let job = docs[0].content;
                        let allJobs = docs[1].content;
                        let procssedJobs = processJob(job, allJobs, build);
                        return processedJobs
                    })
                /*return _get('greenboard', doc_id)
                    .then((doc) => {console.log(doc); result[doc_id] = doc})
                    .then(() => _get('greenboard', existing_builds_id)
                        .then((doc) => {console.log(doc); result[existing_builds_id] = doc}))
                    .then(() => {
                        console.log(result);
                        let job = result[doc_id].content;
                        let allJobs = result[existing_builds_id].content;
                        let procssedJobs = processJob(job, allJobs, build);
                        return processedJobs
                    });
                /*return new Promise(function(resolve, error){
                    let result = {}
                    _get('greenboard', doc_id)
                        .then((doc)=> {
                            console.log(doc);
                            result[doc_id] = doc
                        });
                    _get('greenboard', existing_builds_id)
                        .then((doc) => {
                            console.log(doc);
                            result[existing_builds_id] = doc;
                        });
                    //result[existing_builds_id] = _get('greenboard', existing_builds_id);
                    console.log(result);
                    var job = result[doc_id].content;
                    var allJobs = result[existing_builds_id].content;
        ...
        claimJobs: async function(type,bucket,name,build_id,claim,os,comp,version){
            ...
            const jobs = await _get('greenboard', jobs_key).then(jobs => jobs.value);
            //const jobs = await _getmulti("greenboard", [jobs_key]).then(jobs => jobs[jobs_key].value);
            let triage_history;
            try {
                triage_history = await _get('triage_history', triage_history_key).then(triage_history => triage_history.value);
        ...
        getBuildSummary: function (buildId) {
            function getBuildDetails() {
                return new Promise(function (resolve, error) {
                    let result = {};
                    result[buildId] = _get('greenboard', buildId).then(buildSummary => buildSummary.value);
                    result['existing_builds'] = _get('greenboard', 'existing_builds').then(buildSummary => buildSummary.value);
        ...
        setBestRun: async function(bucket, name, build_id, os, comp, version) {
            ...
            while (true) {
                try {
                    const res = await _get('greenboard', key);
